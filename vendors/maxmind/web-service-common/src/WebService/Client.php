...
namespace HCaptcha\Vendors\MaxMind\WebService;

use HCaptcha\Vendors\Composer\CaBundle\CaBundle;
use HCaptcha\Vendors\MaxMind\Exception\AuthenticationException;
use HCaptcha\Vendors\MaxMind\Exception\HttpException;
use HCaptcha\Vendors\MaxMind\Exception\InsufficientFundsException;
use HCaptcha\Vendors\MaxMind\Exception\InvalidInputException;
use HCaptcha\Vendors\MaxMind\Exception\InvalidRequestException;
use HCaptcha\Vendors\MaxMind\Exception\IpAddressNotFoundException;
use HCaptcha\Vendors\MaxMind\Exception\PermissionRequiredException;
use HCaptcha\Vendors\MaxMind\Exception\WebServiceException;
use HCaptcha\Vendors\MaxMind\WebService\Http\RequestFactory;
/**
...
class Client
{
    ...
    public function __construct(int $accountId, string $licenseKey, array $options = [])
    {
        ...
        $this->useHttps = $options['useHttps'] ?? \true;
    ...
    {
        $headers = [...$headers, 'Authorization: Basic ' . base64_encode($this->accountId . ':' . $this->licenseKey), 'Accept: application/json'];
