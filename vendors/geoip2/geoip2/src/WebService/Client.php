...
namespace HCaptcha\Vendors\GeoIp2\WebService;

use HCaptcha\Vendors\GeoIp2\Exception\AddressNotFoundException;
use HCaptcha\Vendors\GeoIp2\Exception\AuthenticationException;
use HCaptcha\Vendors\GeoIp2\Exception\GeoIp2Exception;
use HCaptcha\Vendors\GeoIp2\Exception\HttpException;
use HCaptcha\Vendors\GeoIp2\Exception\InvalidRequestException;
use HCaptcha\Vendors\GeoIp2\Exception\OutOfQueriesException;
use HCaptcha\Vendors\GeoIp2\Model\City;
use HCaptcha\Vendors\GeoIp2\Model\Country;
use HCaptcha\Vendors\GeoIp2\Model\Insights;
use HCaptcha\Vendors\GeoIp2\ProviderInterface;
use HCaptcha\Vendors\MaxMind\WebService\Client as WsClient;
/**
...
class Client implements ProviderInterface
{
    ...
    private function responseFor(string $endpoint, string $class, string $ipAddress): Country
    {
        ...
        } catch (\HCaptcha\Vendors\MaxMind\Exception\IpAddressNotFoundException $ex) {
            throw new AddressNotFoundException($ex->getMessage(), $ex->getStatusCode(), $ex);
        } catch (\HCaptcha\Vendors\MaxMind\Exception\AuthenticationException $ex) {
            throw new AuthenticationException($ex->getMessage(), $ex->getStatusCode(), $ex);
        } catch (\HCaptcha\Vendors\MaxMind\Exception\InsufficientFundsException $ex) {
            throw new OutOfQueriesException($ex->getMessage(), $ex->getStatusCode(), $ex);
        } catch (\HCaptcha\Vendors\MaxMind\Exception\InvalidRequestException $ex) {
            throw new InvalidRequestException($ex->getMessage(), $ex->getErrorCode(), $ex->getStatusCode(), $ex->getUri(), $ex);
        } catch (\HCaptcha\Vendors\MaxMind\Exception\HttpException $ex) {
            throw new HttpException($ex->getMessage(), $ex->getStatusCode(), $ex->getUri(), $ex);
        } catch (\HCaptcha\Vendors\MaxMind\Exception\WebServiceException $ex) {
