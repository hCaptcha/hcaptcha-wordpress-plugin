...
class EventsTest extends HCaptchaWPTestCase {
	...
	public function test_get_dashboard_data(): void {
		...
		$this->insert_event(
			[
				'status'         => Events::STATUS_TRASH,
				'trashed_at_gmt' => gmdate( 'Y-m-d H:i:s' ),
			]
		);

		$actual = Events::get_dashboard_data();
		...
		self::assertSame( '["WordPress"]', $actual['top_forms'][0]['source'] );
		self::assertSame( 'login', $actual['top_forms'][0]['form_id'] );
		self::assertSame( 2, $actual['top_forms'][0]['total'] );
		self::assertSame( 1, $actual['top_forms'][0]['failed'] );
		self::assertSame( 50.0, $actual['top_forms'][0]['failed_rate'] );

		$top_errors = [];

		foreach ( $actual['top_errors'] as $error ) {
			$top_errors[ $error['code'] ] = $error['total'];
		}

		self::assertSame( 1, $top_errors['empty'] );
		self::assertSame( 1, $top_errors['invalid-or-already-seen-response'] );
		self::assertSame( 3, $actual['peak']['total'] );
		self::assertSame( 1, $actual['peak']['bucket_count'] );
		self::assertSame( 42, $actual['risk']['score'] );
		self::assertSame( 'elevated', $actual['risk']['level'] );
	}
	/**
	 * Test create_table().
	 *
